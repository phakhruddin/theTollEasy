...
//Rare exceptions to indicate a bug in the _thetolleasy code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_thetolleasy = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_thetolleasy]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.thetolleasy.module"
